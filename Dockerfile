...
RUN echo "deb [arch=amd64] http://ppa.launchpad.net/stebbins/handbrake-git-snapshots/ubuntu xenial main" | tee /etc/apt/sources.list.d/handbrake.list
RUN echo "deb [arch=amd64] http://www.deb-multimedia.org jessie main non-free" | tee /etc/apt/sources.list.d/ffmpeg.list
...
RUN rm key.dpkg
RUN apt-get update
RUN apt-get install -y ffmpeg handbrake-cli
